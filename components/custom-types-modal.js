        ...
        <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 dark:bg-black/60 backdrop-blur-sm" role="dialog" aria-modal="true" @click.self="$emit('close')">
          <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-2xl w-full max-w-lg max-h-[80vh] overflow-y-auto border border-slate-200 dark:border-slate-700 m-4">
            <div class="flex items-center justify-between mb-4">
              ...
              <button @click="$emit('close')" class="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 text-xl px-1">&times;</button>
            </div>
            <div class="bg-slate-50 dark:bg-slate-900/50 rounded-xl p-4 border border-slate-200 dark:border-slate-700 mb-4 space-y-3">
              ...
              <div class="flex gap-2">
                <button v-if="editId" @click="$emit('save-edit')" class="flex-1 bg-indigo-600 text-white font-bold py-2 rounded-lg hover:bg-indigo-700 transition text-sm" :disabled="!editName.trim()">Guardar cambios</button>
                <button v-if="editId" @click="$emit('cancel-edit')" class="flex-1 bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300 font-bold py-2 rounded-lg hover:bg-slate-300 dark:hover:bg-slate-600 transition text-sm">Cancelar</button>
                <button v-else @click="$emit('add-type')" class="flex-1 bg-indigo-600 text-white font-bold py-2 rounded-lg hover:bg-indigo-700 transition text-sm" :disabled="!editName.trim()">A\u00f1adir tipo</button>
              </div>
            </div>
            ...
            <div v-for="t in types" :key="t.id" class="flex items-center gap-3 py-2 border-b border-slate-100 dark:border-slate-700/50 last:border-0">
              ...
              <div class="flex gap-1"><button @click="$emit('edit-type', t)" class="text-xs text-indigo-600 hover:underline">Editar</button><button @click="$emit('delete-type', t.id)" class="text-xs text-red-500 hover:underline">Borrar</button></div>
            </div>
          </div>
        </div>
      </Transition>
