      ...
      <div class="tab-fade-in bg-white dark:bg-slate-800 rounded-2xl p-5 border border-slate-200 dark:border-slate-700">
        <div class="flex flex-wrap items-center justify-between gap-2 mb-4">
          ...
          <div class="flex items-center gap-2 flex-wrap">
            ...
            <button @click="root.addConcept()" class="min-h-[44px] sm:min-h-[36px] bg-indigo-600 text-white text-xs font-bold px-3 py-1.5 rounded-lg hover:bg-indigo-700 transition flex-shrink-0">{{ GC.t('concept.add') }}</button>
            <button @click="root.toggleBulkTag()"
                     ...
                     class="min-h-[44px] sm:min-h-[36px] text-xs border rounded-lg px-2 py-1.5 hover:bg-slate-50 dark:hover:bg-slate-700 transition"><svg class="w-3 h-3 inline-block -mt-0.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9.568 3H5.25A2.25 2.25 0 003 5.25v4.318c0 .597.237 1.17.659 1.591l9.581 9.581c.699.699 1.78.872 2.607.33a18.095 18.095 0 005.223-5.223c.542-.827.369-1.908-.33-2.607L11.16 3.66A2.25 2.25 0 009.568 3z"/><path stroke-linecap="round" stroke-linejoin="round" d="M6 6h.008v.008H6V6z"/></svg> {{ GC.t('concept.bulk') }}</button>
            <button @click="root.exportConceptsJSON()" class="min-h-[44px] sm:min-h-[36px] text-xs bg-white dark:bg-slate-800 border dark:border-slate-600 rounded-lg px-2 py-1.5 hover:bg-slate-50 dark:hover:bg-slate-700 transition" title="Exportar JSON"><svg class="w-3 h-3 inline-block -mt-0.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12 12 16.5m0 0L7.5 12m4.5 4.5V3"/></svg> Exportar</button>
            <button @click="root.importConceptsJSON()" class="min-h-[44px] sm:min-h-[36px] text-xs bg-white dark:bg-slate-800 border dark:border-slate-600 rounded-lg px-2 py-1.5 hover:bg-slate-50 dark:hover:bg-slate-700 transition" title="Importar JSON"><svg class="w-3 h-3 inline-block -mt-0.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 16.5V3m0 0L7.5 7.5M12 3l4.5 4.5M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5"/></svg> Importar</button>
          </div>
        </div>
        <div v-if="root.bulkTagMode" class="flex items-center gap-2 mb-3 p-3 bg-amber-50 dark:bg-amber-900/20 rounded-xl border border-amber-200 dark:border-amber-800/40">
          ...
          <button @click="root.applyBulkTag()" class="bg-amber-600 text-white text-xs font-bold px-3 py-1.5 min-h-[44px] rounded-lg hover:bg-amber-700 transition">Aplicar</button>
          <button @click="root.toggleBulkTag()" class="text-xs text-slate-400 hover:underline min-h-[36px]">Cancelar</button>
        </div>
        ...
        <div class="space-y-1">
          <div v-for="c in filteredConcepts" :key="c.id"
            ...
            <button @click="root.openInspector(c.id)" class="min-w-[44px] min-h-[44px] sm:min-w-[28px] sm:min-h-[28px] flex items-center justify-center text-indigo-500 hover:text-indigo-700 dark:hover:text-indigo-300 transition rounded-lg hover:bg-indigo-50 dark:hover:bg-indigo-900/20" title="Inspector">
            ...
            <button @click="root.editConcept(c.id)" class="min-w-[44px] min-h-[44px] sm:min-w-[28px] sm:min-h-[28px] flex items-center justify-center text-indigo-500 hover:text-indigo-700 dark:hover:text-indigo-300 transition rounded-lg hover:bg-indigo-50 dark:hover:bg-indigo-900/20">&#9998;&#65039;</button>
            <button @click="root.deleteConcept(c.id)" class="min-w-[44px] min-h-[44px] sm:min-w-[28px] sm:min-h-[28px] flex items-center justify-center text-red-400 hover:text-red-600 dark:hover:text-red-300 transition rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20">&#10005;</button>
          </div>
        </div>
        <div class="mt-6 pt-4 border-t border-slate-200 dark:border-slate-700">
          <div class="flex items-center justify-between mb-3">
            ...
            <div class="flex gap-2">
              <button @click="root.showRelationForm = !root.showRelationForm"
                       class="text-xs bg-indigo-600 text-white font-bold px-3 py-1.5 min-h-[44px] rounded-lg hover:bg-indigo-700 transition"
                       v-text="root.showRelationForm ? 'Cerrar' : '+ Nueva relaci\u00f3n'"></button>
              <button @click="root.openCustomTypes()"
                       class="text-xs bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 font-bold px-3 py-1.5 min-h-[44px] rounded-lg hover:bg-slate-50 dark:hover:bg-slate-600 transition">Gestionar tipos</button>
            </div>
          </div>
          <div v-if="root.showRelationForm"
            ...
            <button @click="root.addRelationFromForm()" class="w-full bg-indigo-600 text-white text-sm font-bold py-2 min-h-[44px] rounded-lg hover:bg-indigo-700 transition" :disabled="!root.relFormFrom || !root.relFormTo">Crear relacione</button>
          </div>
          <div class="space-y-2">
            <div v-for="(r, i) in subject.relations" :key="i"
              ...
              <button @click="root.deleteRelation(i)" class="ml-auto text-xs text-red-500 dark:text-red-400 hover:underline min-w-[36px] min-h-[36px] flex items-center justify-center">&#10005;</button>
            </div>
          </div>
