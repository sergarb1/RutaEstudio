      ...
      <header class="flex items-center justify-between gap-2 flex-wrap mb-8">
        ...
        <div class="flex items-center gap-1.5 flex-wrap justify-end">
          <button @click="$emit('toggle-dark')"
                  class="min-w-[40px] min-h-[40px] sm:min-w-[36px] sm:min-h-[36px] flex items-center justify-center bg-white dark:bg-slate-800 border dark:border-slate-600 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 transition"
          ...
          <select @change="$emit('set-lang', $event.target.value)" :value="lang"
                  class="min-h-[40px] sm:min-h-[36px] text-xs bg-white dark:bg-slate-800 border dark:border-slate-600 rounded-lg px-2 font-medium transition cursor-pointer focus:outline-none">
            <option value="es">ES</option>
            <option value="en">EN</option>
            <option value="va">VA</option>
          </select>
          <button v-if="canInstall" @click="$emit('install')"
                  class="flex items-center gap-1.5 text-xs font-medium bg-indigo-100 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800/60 rounded-xl px-3 py-2.5 sm:py-2 hover:bg-indigo-200 dark:hover:bg-indigo-800/60 transition"
          ...
          <button @click="$emit('import-data')"
                  class="flex items-center gap-1.5 text-xs font-medium bg-white dark:bg-slate-800 border dark:border-slate-600 rounded-xl px-3 py-2.5 sm:py-2 hover:bg-slate-50 dark:hover:bg-slate-700 transition"
